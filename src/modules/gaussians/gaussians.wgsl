...
fn main_vertex(vertex : VertexIn) -> VertexOut {
	...
	var viewMat3 = mat3x3f(
		uniforms.view[0].xyz,
		uniforms.view[1].xyz,
		uniforms.view[2].xyz
	);
	var cov3DView = viewMat3 * cov3D * transpose(viewMat3);

    var focal = vec2f(
        uniforms.proj[0][0] * uniforms.screen_width * 0.5f,
        uniforms.proj[1][1] * uniforms.screen_height * 0.5f
    );
    var s = 1.0f / abs(viewPos.z * viewPos.z);

    // Direct screen-space covariance projection
    var cov2Dm = mat2x2f(
        cov3DView[0][0] * focal.x * focal.x * s,
        cov3DView[0][1] * focal.x * focal.y * s,
        cov3DView[1][0] * focal.x * focal.y * s,
        cov3DView[1][1] * focal.y * focal.y * s
    );
    cov2Dm[0][0] += 0.5f;
    cov2Dm[1][1] += 0.5f;
	...
	{
        var depth = abs(viewPos.z);
        var pixelSize = depth / uniforms.screen_height;
        var dx = pixelSize;
        var dy = pixelSize;
